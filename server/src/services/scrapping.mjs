...
export const main = async (user, password) => {
  ...
  try {
    await goData(page, "#link-1-meus-dados", 'a[data-tab="boletim"]');

    console.log("✅ Navegou para boletins!");
    const boletim = await getDataAcademy(page, "table.borda");
    await browser.close();
...
const goData = async (page, bottomSelector, dataSelector) => {
...
const getDataAcademy = async (page, table) => {
